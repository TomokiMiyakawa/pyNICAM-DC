// ROCm/RCCL port of ncclffi.cu (hipified). RCCL is ABI-compatible with NCCL
// (same nccl* symbols), so ONLY the CUDA runtime calls + stream type + headers
// change; every ncclXXX call is byte-identical to the CUDA source. Build with
// tools/ncclffi/build_ncclffi_rocm.sh (hipcc, -lrccl). Register the FFI target as
// platform="ROCM" (mod_ncclffi.py auto-detects AMD).
...
extern "C" int ncclffi_init(int rank, int nranks, const char* uid_bytes, int device) {
  hipError_t ce = hipSetDevice(device);
  if (ce != hipSuccess) return 1000 + (int)ce;
  ncclUniqueId id;
...
static ffi::Error PeerExchangeImpl(hipStream_t stream, ffi::Buffer<ffi::F32> x,
...
static ffi::Error HaloExchangeImpl(hipStream_t stream, ffi::AnyBuffer x,
  ...
  if (g_sync) hipDeviceSynchronize();
  ...
  hipMemcpyAsync(tok_out->untyped_data(), tok_in.untyped_data(), sizeof(float),
  ...
  if (g_cksum) {
    ...
    hipMemcpy(hx.data(), xb, xb_bytes, hipMemcpyDeviceToHost);
    hipMemcpy(hy.data(), yb, yb_bytes, hipMemcpyDeviceToHost);
    for (size_t i = 0; i < p.peers.size(); ++i) {
