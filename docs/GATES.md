...
| `PYNICAM_COMM_IOCB` | 0 | choice | halo exchange via mpi4py from an ordered io_callback — in-graph, no mpi4jax. ~3% slower than mpi4jax on CPU (gl05 pe8, bit-exact); host-staged by construction, so **not** a GPU substitute |
